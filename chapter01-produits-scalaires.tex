\chapter{Produits scalaires et orthogonalité}
\label{cha:prod-scal-et}
 
\hrule
\medskip
\begin{center}
  Cours du 23. Février 2016
\end{center}
\hrule 
...
\begin{definition}
  ...
  Un \emph{produit scalaire} sur $V$
  est une correspondance qui, à tout couple $(v,w)$
  d'éléments de $V$
  associe un scalaire, noté $\pscal{v,w}$,
  ou encore $v\cdot w$, satisfaisant aux propriétés suivantes:
  \begin{enumerate}[PS 1]
  \item On a $\pscal{v,w} = \pscal{w,v}$ pour tout $v,w \in V$.  \label{ps:1}
  \item Si $u,v$ et $w$ sont des éléments de $V$, \label{ps:2}
    \begin{displaymath}
      \pscal{u,v+w} = \pscal{u,v}+\pscal{u,w} 
    \end{displaymath}
  \item Si $x \in K$ et $u,v \in V$,  \label{ps:3}
    \begin{displaymath}
       \pscal{u,x \,v} = x \pscal{u,v}. 
    \end{displaymath}  
  \end{enumerate}
  \noindent
On dit que le produit scalaire est \emph{non dégénéré} si, de plus, la condition suivante est vérifiée:
\begin{quote}
...
\begin{example}
 \label{ex:1}
  ...
  \begin{displaymath}
    \begin{array}{rcl}
      \pscal{\,}\colon    \,  V\times V& \longrightarrow &V \\
                              (u,v)   & \longmapsto & \sum_{i=1}^n u_i v_i
    \end{array}  
  \end{displaymath}
est un produit scalaire. 
Vérifions (\ref{ps:2}): $\pscal{u,v+w} = \sum_{i=1}^n u_i (v_i+w_i) = \sum_{i=1}^n u_i v_i + u_i w_i = \sum_{i=1}^n u_iv_i +\sum_{i=1}^n u_iw_i =\pscal{u,v} + \pscal{u,w}$. 
\end{example}
...
\begin{exercise}
  \label{ex:3}
  Montrer que les produits scalaires des exemples~\ref{ex:1} et \ref{ex:2} sont non dégénérés. 
\end{exercise}

\begin{definition}
  \label{def:2}
  Soit $V$ un espace vectoriel muni d'un produit scalaire. Deux éléments $u,v \in V$ sont \emph{orthogonaux} ou \emph{perpendiculaires} si $\pscal{u,v} = 0$, et l'on écrit $v \perp w$. 
\end{definition}
...
\noindent 
  ...
  \begin{displaymath}
    \pscal{e,u-v} = \pscal{e,u} + \pscal{e,-v} = 0 + (-1)\cdot \pscal{e,v} = 0,
  \end{displaymath}  
  et pour $c \in K$ 
  \begin{displaymath}
...
\begin{example}
  ...
  \end{displaymath}
  où les $A_i$  sont les vecteurs lignes de la matrice $A$. Soit $W$ le sous-espace de $K^n$ engendré par les $A_i$ et $U$ le sous-espace de $K^n$ des solutions du système~\eqref{eq:1}. Alors on a  $U = W^\perp$ et $\dim(W^\perp) = \dim(U) = n - \rank(A) = \dim(\noy(A))$. 
...
\begin{definition}
  \label{def:4}
  Soit $V$ un espace vectoriel sur $\R$ muni d'un produit scalaire.  
  Le produit scalaire est défini positif si $\pscal{v,v} \geq 0$ pour tout $v \in V$, et si $\pscal{v,v}>0$ lorsque $v \neq 0$. 
\end{definition}

\begin{example}
  \label{exe:3}
  Soit $V = \R^n$. Le \emph{produit scalaire ordinaire} 
  \begin{displaymath}
    \pscal{u,v} = \sum_{i=1}^n u_iv_i 
  \end{displaymath}
  est défini positif. Aussi le produit scalaire de l'exemple~\ref{ex:2} est défini positif. 
\end{example}

\begin{definition}
  \label{def:5}
  Soit $\pscal{\,}$ un produit scalaire défini positif. La \emph{longueur} ou la \emph{norme} d'un élément $v \in V$ est le nombre 
  \begin{displaymath}
    \| v \| = \sqrt{\pscal{v,v}}.
  \end{displaymath}
...
\begin{framed}\noindent 
  Pour le reste de ce paragraphe~\ref{sec:le-case-reel}, si pas spécifié autrement,  $V$
  est toujours un espace vectoriel sur $\R$
  muni d'un produit scalaire défini positif.
\end{framed}
...
\begin{proposition}[Règle du parallélogramme] Pour tout $v$ et $w$, on a 
  \begin{displaymath}
...
\begin{definition}
  \label{def:6}
  Soit $V$ un espace vectoriel sur un corps $K$, muni d'un produit scalaire. 
  Soit $w \in V$ tel que  $\pscal{w,w} > 0$. Pour $v \in V$, soit $\alpha = \pscal{v,w} / \pscal{w,w}$.  Le nombre $\alpha$ est la \emph{composante} de $v$ sur $w$, ou \emph{le coefficient de Fourier de $v$ relativement à $w$}. Le vecteur $\alpha \, w$ s'appelle la \emph{projection} de $v$ sur $w$. 
\end{definition}
...
\begin{proof}
  Si $w=0$ les deux termes de cette inégalité sont nuls et elle devient évidente. Supposons maintenant que $w$ est un vecteur unitaire. Si $\alpha = \pscal{v,w}$ est la composante de $v$ sur $w$, $v - \alpha w$ est perpendiculaire à $w$ donc aussi a $\alpha\,w$. D'après le théorème de Pythagore on trouve 
  \begin{eqnarray*}
...
\hrule 
\begin{center}
  Cours du 1. Mars  2016
\end{center}
\hrule 

\medskip 
\noindent 
On appelle un  espace vectoriel sur $\R$ muni d'un produit scalaire défini positif un \emph{espace euclidien}.  
...
\begin{lemma}
  ...
  soient $v_1,\dots,v_n$ des éléments de V, deux à deux orthogonaux et soit $\pscal{v_i,v_i}\neq 0$ pour tout $i$. % Soit $\alpha_i = \pscal{v,v_i}/\pscal{v_i,v_i}$ la composante de $v$ sur $v_i$, alors 
Un vecteur 
\begin{displaymath}
  v - a_1v_1- \cdots - a_n v_n\, \text{ où  }\, a_i \in K 
\end{displaymath}
est perpendiculaire à tout $v_1,\dots,v_n$ si et seulement si $a_i$ est la composante de $v$ sur $v_i$, i.e., $a_i=  \pscal{v,v_i}/ \pscal{v_i,v_i}$ pour tout $i$. 
\end{lemma}
...
\begin{notation}
  Soit $V$ un espace vectoriel et $v_1,\dots,v_n \in V$. Le sous-espace engendré par $v_1,\dots,v_n$ est dénoté par  $\spa\{v_1,\dots,v_n\}$. 
\end{notation}



\begin{theorem}[La procédé d'orthogonalisation de Gram-Schmidt]
  \label{thr:2}
  Soit $V$ un espace euclidien et  $\{v_1,\dots,v_n\} \subseteq V$
  un ensemble libre.  
...
\begin{proof}
  ...
  On met  
  \begin{displaymath}
    u_i = v_i - \alpha_{1i}  u_1 - \cdots \alpha_{i-1,i} u_{i-1}
  \end{displaymath}
  où les $\alpha_{ji}$
...
\begin{definition}
  ...
  orthogonale et se compose des vecteurs unitaires seulement.
\end{definition}
...
\item Soit $V$ un espace vectoriel muni d'un produit scalaire $\pscal{.}$. Soit $U \subseteq V$ un sous-espace. Montrer que $\pscal{.}$ restreint à $U$ est un produit scalaire du sous-espace $U$. 
\item Soient $V$ un espace vectoriel muni d'un produit scalaire $\pscal{.}$ et $\{v_1,\dots,v_n\} \subseteq V$ un ensemble deux à deux orthogonal. 
  \begin{enumerate}[a)]
  \item   Montrer que $\{v_1,\dots,v_n\}$ est un ensemble libre si pour tout $i$, 
 $\pscal{v_i,v_i} \neq  0$.  
\item  Donner un contre-exemple ou une démonstration du contraire. 
\end{enumerate}
...
\item Trouver la factorisation $Q\cdot R$  du Corollaire~\ref{co:2} de la matrice 
  ...
  \begin{displaymath}
    ...
    \end{pmatrix}
  \end{displaymath}
\item Trouver un produit scalaire de $\R^n$ tel qu'il existe des vecteurs $u,v \in \R^n$ avec $\pscal{u,u}<0$ et $\pscal{v,v}>0$. 
\item Soit $V$ un espace vectoriel sur $\R$ muni d'un produit scalaire. S'il existe des vecteurs $u,v \in V$ tels que $\pscal{u,u}<0$ et $\pscal{v,v}>0$, il existe un vecteur $w \neq 0$ tel que $\pscal{w,w}=0$. 
...
\item On considère l'espace euclidien des fonctions continues sur l'intervalle $[0,1]$ muni du produit scalaire 
  \begin{displaymath}
...
\begin{equation}
  ...
  \min_{x \in \R^n} \|Ax - b\|. 
\end{equation}
Pour le reste de ce paragraphe, $V$ est toujours  un espace euclidien. 
...
\section{Bases orthogonales, le cas général}
\label{sec:bases-orth-le}
...
\begin{notation}
  ...
  $\Z / n \Z$ ou plus brièvement $\Z_n$. Ceci est un corps si et seulement si $n$ est un nombre premier. 
\end{notation}
...
\begin{lemma}
  \label{lem:1}
  Soit $V$ un espace vectoriel sur le corps $K$, $\mathrm{Char}(K)\neq 2$ et soit $\pscal{.}$ un produit scalaire de $V$. Si $\pscal{u,u}=0$ pour tout $u \in V$ alors 
  \begin{displaymath}
    \pscal{u,v}=0 \text{ pour tous }u,v \in V
  \end{displaymath}
On dit que le produit scalaire est \emph{nul}. 
\end{lemma}
...
\begin{theorem}
  \label{thr:5}
  Soit $V$ un espace vectoriel sur le corps $K$,  avec $\mathrm{Char}(K)\neq 2$ et supposons que $V\neq \emptyset$ est de dimension finie et  muni d'un produit scalaire. Alors $V$ possède une base orthogonale.
\end{theorem}
...
Soit $\dim(V) >1$.  Si $\pscal{u,u} = 0$ pour tout $u$, le lemme~\ref{lem:1} implique que le produit scalaire est nul et toute base de $V$  est orthogonale.  
...
\end{displaymath}
alors $V = V_1 + V_1^\perp$. Maintenant on montre que cette somme est directe parce que chaque élément de $V_1 \cap V_1^\perp$ s'écrit comme $\beta \cdot u$ pour $\beta \in K$. Et $\pscal{u,\beta u} = \beta \pscal{u,u} = 0$ implique $\beta = 0$. 

Alors $\dim(V^\perp) < \dim(V)$, et par induction, $V^\perp$ possède une base orthogonale $\{v_2,\dots,v_n\}$. Alors $\{v_1,\dots,v_n\}$ est une base orthogonale de $V$. 
\end{proof}  
...
\text{ et }
 v_3 =
  ...
  \end{displaymath}
Est-ce que $\spa\{v_1,v_2,v_3\}$ possède une base orthogonale par rapport au produit scalaire standard de l'exemple~\ref{ex:1}? 

\item En considérant le produit scalaire standard de l'exemple~\ref{ex:1}, trouver une base orthogonale du sous-espace de $\Z_3^4$ engendré par 
  \begin{displaymath}
    v_1 =
    \begin{pmatrix}
      1\\1\\1\\0
...
\begin{definition}
\label{def:9}
  L'ensemble des  applications linéaires  de $V$ dans $K$ est noté $V^*$ et, muni de l'addition et de la multiplication scalaire usuelles, est appelé l'\emph{espace dual} de $V$. Les éléments de $V^*$ sont des \emph{formes linéaires}.
\end{definition}

\begin{example}
  
Si $\pscal{.}$ est un produit scalaire de $V$ et $v \in V$, alors $f(x) = \pscal{x,v} \in V^*$ est une forme linéaire. 
\end{example}


Si $V$ est de dimension finie et si $B = \{V_1,\dots,v_n\}$ est une base de $V$ l'image d'un vecteur $x = \sum_i\alpha_i v_i$ est 
\begin{eqnarray*}
...
\begin{proof}
 ...
 0 =    \left(\sum_i \beta_i \phi_i\right) (v_j) =  \beta_j,
  \end{displaymath}
c'est-à-dire  les $\phi_j$ sont linéairement indépendantes. Les $\phi_j$ engendrent $V^*$ dès que pour $f \in V^*$, $f = \sum_i  f(v_i)  \, \phi_i$. 
\end{proof}
...
\begin{definition}
  ...
  \end{displaymath}
  est une forme bilinéaire si pour tout $v \in V$ les applications 
  $g,h: V \longrightarrow K$ telles que $g(x) = f(v,x)$ et $h(x) = f(x,v)$ sont des formes linéaires. 
\end{definition}

\begin{example}
  \label{exe:9}
  Un produit scalaire $\pscal{.}$ est une forme bilinéaire symétrique. 
\end{example}

Soit $V$ un espace vectoriel de dimension finie et $B = \{v_1,\dots,v_n\}$ une base de $V$. Pour une forme bilinéaire $f$ et $x = \sum_i\alpha_i v_i$ et $y = \sum_i \beta_i v_i$ on a 
\begin{displaymath}
  f(x,y) = \sum_{i,j} \alpha_i\beta_j f(v_i,v_j), 
\end{displaymath}
alors pour la matrice $A_B^f \in K^{n\times n}$, ayant comme composantes $f(v_i,v_j)$, on a 
\begin{displaymath}
  f(x,y) = [x]_B^T \,A_B^f \,  [y]_B. 
\end{displaymath}
La forme bilinéaire $f$ est symétrique (i.e. $f(v, w) = f(w, v) \ \forall v, w \in V$) si et seulement si $A_B^f$ est symétrique pour toute base $B$ de $V$, auquel cas c'est un produit scalaire. En fait, il suffit que la matrice de $f$ soit symétrique dans une seule base; elle le sera alors dans toute autre base par la formule du changement de base.
...
\begin{lemma}
  \label{lem:4}
  Soit $V$
  un espace vectoriel sur le corps $K$ de dimension finie muni d'un produit scalaire
  $\pscal{.}$
  Le produit scalaire est non dégénéré si et seulement si la matrice
  $A_B^{\pscal{}}$ est de rang plein pour toute base $B$.
\end{lemma}

\begin{proof}
  Soit $n$ la dimension de $V$. 
  Si le produit scalaire est dégénéré, alors il existe  $0 \neq v \in V$
  tel que $\pscal{v,u}=0$ pour tout $u \in V$. Alors
  \begin{displaymath}
    [v]_B^T A_B^{\pscal{}} x = 0 \, \text{ pour tout } \,  x \in K^n,
  \end{displaymath}
alors $[v]_B^T A_B^{\pscal{}} = 0$ qui démontre $\rank(A_B^{\pscal{}})<n$. 

Si $\rank(A)<n$ alors il existe $0 \neq v \in V$ tel que $[v]_B^TA_B^{\pscal{}} = 0$. Alors, $\pscal{v,u} = 0$ pour tout $u \in V$ qui démontre que $\pscal{.}$ est dégénéré. 
...
\begin{lemma}
  \label{lem:5}
  Soit $V$ un espace vectoriel sur le corps $K$  de dimension finie muni d'un produit scalaire non dégénéré et soit $f\colon V \longrightarrow K$ une forme linéaire. Il existe un $v \in V$ tel que $f(x) = \pscal{v,x}$ pour tout $x \in V$. 
\end{lemma}

\begin{proof}
  Soient $B = \{v_1,\dots,v_n\}$ une base  de $V$ et $A_B^{\pscal{}} \in K^{n\times n}$ la matrice  associée au produit scalaire et à la base $B$. Soit $a \in K^n$ tel que  $f(x) = a^T[x]_B$ pour tout $x \in V$. Dès que $A_B^{\pscal{}}$ est de rang plein, alors il existe $v \in V$ tel que $[v]_B^TA_B^{\pscal{}} = a^T$. Ceci revient à résoudre un système d'équations linéaires (cf semestre 1) et comme la matrice $A_B^{\pscal{}}$ est de rang plein, on a l'existence (et même l'unicité) d'une solution, i.e.,  $[v]_B^TA_B^{\pscal{}} = a^T$. Ainsi 
...
\begin{theorem}[Supplémentaire orthogonal]
  \label{thr:8}
  Soient $V$ un espace vectoriel de dimension finie sur corps $K$ et $W$ un sous-espace de $V$. Soit $\pscal{.}$ un produit scalaire tel que, si restreint sur $W\times W$ est non dégénéré. Alors $V = W \oplus W^\perp$.  
\end{theorem}
...
\item On considère les vecteurs 
 ...
 v_2 =
    ...
    \end{pmatrix} \in \Z_2^4
  \end{displaymath}
et le produit scalaire standard. Trouver une base du $\spa\{v_1,v_2\}^\perp$. Est-ce que $\Z_2^4 = \spa\{v_1,v_2\} \oplus \spa\{v_1,v_2\}^\perp$? 
...
\item Soient $V$ un espace vectoriel de dimension finie sur un corps $K$ et soit $\pscal{.}$ un produit scalaire. Exprimez $(W_1 +W_2)^\perp$ et $(W_1\cap W_2)^\perp$ en fonction de $W_1^\perp$ et $W_2^\perp$. 
...
Soit $V$ un espace vectoriel sur un corps $K$ de dimension finie, muni d'un produit scalaire. Nous avons vu (théorème~\ref{thr:5}) que $V$ possède une base orthogonale. Supposons que cette base est $B = \{v_1,\dots,v_n\}$ et considérons $x = \sum \alpha_i v_i \in V$ et $y \in \sum_i \beta_i v_i \in V$. Le produit scalaire s'écrit 
\begin{eqnarray*}
  \pscal{x,y} & =  & \sum_{ij} \alpha_i \beta_j \pscal{v_i,v_j} \\
...
\begin{definition}
  \label{def:11}
  Le sous espace $V_0 = \{v \in V \colon \pscal{v,x} = 0 \text{ pour tout } x \in V\}$ est appelé l'\emph{espace de nullité} du produit scalaire $\pscal{.}$. 
\end{definition}
...
\begin{theorem}
  ...
  de caractéristique $\neq 2$
  et soit $V$
  muni d'un produit scalaire. Soit $B = \{v_1,\dots,v_n\}$
...
\begin{proof}
  ...
  \begin{displaymath}
    \pscal{v,x} =   [v]_B^T
                    ...
                    \end{pmatrix} [x]_B. 
  \end{displaymath}
La forme linéaire $f(x) = \pscal{v,x}$ est égale à zéro si et seulement si $\left([v]_B\right)_i = 0$ pour tout $i$ tel que $c_i \neq 0$. Ceci démontre que $\{ v_i \colon \pscal{v_i,v_i}=0\}$ est une base de l'espace de nullité. 
\end{proof}
...
muni d'un produit scalaire. Soit $B = \{v_1,\dots,v_n\}$
une base orthogonale de $V$.
Il existe un nombre entier $r \geq 0$
tel que exactement $r$ des indices $i$ satisfont $\pscal{v_i,v_i}>0$.
\end{theorem}
...
\begin{definition}
  \label{def:12}
  L'entier $r$ du théorème de Sylvester est appelé l'\emph{indice de positivité} du produit scalaire. 
\end{definition}
...
\begin{enumerate}
\item Soit $V$ un espace vectoriel de dimension finie sur $\R$ et soit $\pscal{.}$ un produit scalaire sur $V$.  Montrer que $V$ admet une décomposition en somme directe 
  \begin{displaymath}
    V_0 \oplus V^+ \oplus V^-
  \end{displaymath}
...
\section{Classification des matrices} 
\label{sec:class-des-matr}

\begin{definition}
  \label{def:13}
  Soit $K$ un corps. Deux matrices $A,B \in \R^{n\times n}$ sont dites \emph{équivalentes} ou \emph{congruentes} s'il existe une matrice $P \in \R^{n \times n}$ inversible telle que 
\begin{displaymath}
  A = P^T B P.
\end{displaymath}
...
Le relation entre $\cong$ et le concept de l'orthogonalité   est le lemme suivant. 
\begin{lemma}
\label{thr:13}
  Soit $V$ un espace vectoriel sur $K$ de dimension finie, muni d'un produit scalaire et soit $B = \{v_1,\dots,v_n\}$ une base de $V$. $V$ possède une base orthogonale si et seulement s'il existe une matrice diagonale $D$ 
tel que 
 $A_B^{\pscal{.}} \cong D$. 
\end{lemma}
\begin{proof}
...
\end{equation}  égal à zéro si $i \neq j$. Pour la matrice $P \in K^{n \times n}$ dont les colonnes sont les vecteurs $[u_1]_B,\dots,[u_n]_B$ on a alors que 
\begin{displaymath}
  P^T A_B^{\pscal{.}}P
\end{displaymath}
...
\begin{displaymath}
    ...
    \end{pmatrix}. 
  \end{displaymath}
  La base orthogonale est $B' = \{w_1,\dots,w_n\}$ dont 
  \begin{displaymath}
     [w_j]_B =
  \begin{pmatrix}
    p_{1j}\\ \vdots \\ p_{nj}
...
\begin{corollary}
  \label{co:4}
  Soit $K$ un corps de caractéristique différente à $2$. Toute matrice symétrique  $A \in K^{n \times n}$ est congruente à une matrice diagonale. 
\end{corollary}
\begin{proof}
Ceci est un corollaire du  lemme~\ref{thr:5} et du théorème~\ref{thr:13} 
parce que  $K^n$ muni du produit scalaire $\pscal{u,v} = u^TAv$ possède une base orthogonale. 
\end{proof}


\begin{example}
  \label{exe:14}
  Soient $V$ un espace vectoriel sur $\R$ et $\{v_1,v_2,v_3\}$ une base de 
\end{example}



En plus, si $K = \R$ alors $A \in \R^{n \times n}$. Soient les  $c_i =  \pscal{v_i,v_i}$ ordonnés comme  $c_1,\dots, c_r >0$, $c_{r+1},\dots ,c_s<0$ et $c_{s+1},\dots , c_n = 0$. En multipliant les premier $s+r$ vecteurs $v_i$ avec $1/ \sqrt{|c_i|}$ on obtient en fait une base orthogonale  $v_1,\dots,v_r,v_{r+1},\dots,v_s,v_{s+1},\dots,v_n $ de $\R^n$ telle que $c_1,\dots, c_r =1$, $c_{r+1},\dots ,c_s=-1$ et $c_{s+1},\dots , c_n = 0$. 

Alors, si $P \in \R^n$ est la matrice inversible dont les colonnes sont les vecteurs $v_1,\dots,v_n$ ordonnés de la même manière on trouve 
\begin{equation}
  \label{eq:5}
  P^T A P =
  \begin{pmatrix}
...
\noindent 
Nous pouvons noter le corollaire suivant du théorème de Sylvester. 
\begin{corollary}
  \label{co:5}
  Chaque matrice symétrique $A \in \R^{n \times n}$ est congruente à une unique matrice du type \eqref{eq:5}. 
\end{corollary}
...
\begin{displaymath}
   \begin{pmatrix}
      ...
      4 & 4 & 3 \\
      6 & 3 & 1 
    \end{pmatrix} \cdot  \left[\begin{matrix}1 & -2 & -3\\0 & 1 & 0\\0 & 0 & 1\end{matrix}\right] = \left[\begin{matrix}2 & 0 & 0\\4 & -3 & -9\\6 & -9 & -17\end{matrix}\right]. 
\end{displaymath}
Alors 
\begin{displaymath}
  P_1^T\cdot A \cdot P = \left[\begin{matrix}2 & 0 & 0\\0 & -3 & -9\\0 & -9 & -17\end{matrix}\right]
\end{displaymath}
Avec 
\begin{displaymath}
  P_2 = \left[\begin{matrix}1 & 0 & 0\\0 & 1 & -3\\0 & 0 &1\end{matrix}\right]
\end{displaymath}
on obtient 
\begin{displaymath}
  P_2^TP_1^T A P_1 P_2 = \left[\begin{matrix}2 & 0 & 0\\0 & -3 & 0\\0 & 0 & 10\end{matrix}\right]
\end{displaymath}
L'indice de nullité est zéro et l'indice de positivité est $2$. Le produit $P_1\cdot P_2$ est égal à 
\begin{displaymath}
  P_1 \cdot P_2 = \left[\begin{matrix}1 & -2 & 3\\0 & 1 & -3\\0 & 0 & 1\end{matrix}\right]
\end{displaymath}
Les colonnes sont une base de Sylvester. En divisant les colonnes par $\sqrt{2},\sqrt{3}$ et $\sqrt{10}$ respectivement, et en échangeant les deux dernières colonnes, on obtient une transformation $P$ telle que $P^TAP =
\begin{pmatrix}
  1& \\
  & 1 & \\
  & & -1
...
\begin{example}
  \label{exe:15}
  Soit $V$ une espace vectoriel sur $\Q$ de dimension $3$ muni d'un produit scalaire. Soit $B = \{v_1,v_2,v_3\}$ une base de $V$ et 
  \begin{displaymath}
    A^{\pscal{.}}_B =
    \begin{pmatrix}
      1 & 0 & 2\\
...
\item Est-ce que la matrice 
  ...
  \end{displaymath}
est congruente à une matrice diagonale? \emph{Renseignement: voir l'exercice~\ref{item:1}. de la section~\ref{sec:bases-orth-le}.}  
\item Soit $V$ un espace vectoriel  sur un corps $K$ de dimension finie, muni d'un produit scalaire $\pscal{.}$. Soit $B = \{v_1,\dots,v_n\}$ une base de $V$. Montrer que $A_B^{\pscal{.}} \in K^{n \times n}$ est congruente à une matrice diagonale si  et seulement si $V$ possède une base orthogonale. 

\item Soit $K$ un corps de caractéristique $2$ et soit $V$ un espace vectoriel sur $K$ de dimension finie, muni d'un produit scalaire. Soit 
  \begin{displaymath}
    C =
    \begin{pmatrix}
      0 & 1 \\
...
\item Comment peut on déterminer si un espace vectoriel de dimension finie muni d'un produit scalaire possède une base orthogonale? Décrire très brièvement une méthode. 
\item Déterminer l'indice de nullité et l'indice de positivité des produits scalaires définis par les matrices suivantes
  \begin{displaymath}
    \begin{pmatrix}
      1 & 2 \\
      2 & -1
...
\item Soit $V$ un espace euclidien de dimension $n$. Montrer que $V$ possède une base $B$ telle que pour tout $x,y \in V$
  ...
  \end{displaymath}
où $ [x]_B \cdot [y]_B$  dénote le produit scalaire standard de $\R^n$ entre $[x]_B$ et $ [y]_B$ .  
...
Maintenant nous considérons le cas $K = \C$. Il faut un peu modifier la définition d'un produit scalaire pour obtenir des résultats similaires à ceux des sections précédentes.  
Le carré de la longueur d'un vecteur 
\begin{displaymath}
  v =
  \begin{pmatrix}
    a_1 + i\cdot  b_1 \\
...
\begin{definition}
  \label{def:15}
  Soit $V$
  un espace vectoriel sur un corps $\C$.
  Un \emph{produit hermitien} sur $V$
  est une correspondance qui, à tout couple $(v,w)$
  d'éléments de $V$
  associe un nombre complexe, noté $\pscal{v,w}$, satisfaisant les propriétés suivantes:
\medskip 
  ...
  \end{enumerate}
  \noindent
On dit que le produit hermitien  est \emph{non dégénéré} si, de plus, la condition suivante est vérifiée:
\begin{quote}
  Si $v \in V$ et si $\pscal{v,w}=0$ pour tout $w \in V$, alors $v = 0$. 
\end{quote}

Pour tout $v \in V$ $\pscal{v,v} \in \R$, dès que $\pscal{v,v} = \overline{\pscal{v,v}}$.  
On dit que le produit hermitien  est \emph{défini positif} si $\pscal{v,v} \geq 0$ pour tout $v \in V$ et si $\pscal{v,v}>0$ lorsque $v \neq 0$. 
...
Les notions d'\emph{orthogonalité, de perpendicularité, de base orthogonale} et \emph{de supplémentaire orthogonal}  sont définies comme avant, de même que les notions de \emph{coefficients de Fourier} et la projection de $v$ sur $w$. 
...
\begin{definition}
  \label{def:16}
  Une application $f \colon V \times V \longrightarrow \C$ satisfaisant

  PH~\ref{ph2} et PH~\ref{ph3} est appelé une \emph{forme sesquilinéaire}. 
\end{definition}
...
\begin{proposition}
  \label{prop:3}
  Soit  $V$  un espace vectoriel sur $\C$ de dimension finie et soit $B$  une base de $V$. Une forme sesquilinéaire $f$ est un produit hermitien si et seulement si $A_B^f$ est hermitienne. 
\end{proposition}
...
\begin{theorem}
  \label{thr:11}
  Soit $V$ un espace vectoriel sur $\C$ de dimension finie, muni d'un produit hermitien. Alors $V$ possède une base orthogonale. 
\end{theorem}
\begin{proof}
...
\end{displaymath}
est une matrice diagonale. Nous échangeons la première et deuxième colonnes ainsi que la première et deuxième lignes et obtenons 
\begin{displaymath}
...
\item Soit $V$ un espace vectoriel sur $\C$ et $f\colon V\times V \longrightarrow \C$ une application satisfaisant les axiomes 
  ...
  \item Si $x \in \C$ et $u,v \in V$, 
    ...
    \end{displaymath}  
  \end{enumerate}
Montrer que $f$ est un produit hermitien. 
...
\item Soit $V$ un espace vectoriel sur $\C$ de dimension $n$ et soit $\pscal{.}$ un produit hermitien. Montrer que $\pscal{.}$ est non dégénéré si et seulement si $\rank(A_B^{\pscal{.}}) = n$  pour chaque base $B$ de $V$. 
...
\item Soient $V$ un espace vectoriel sur $\C$ et $\dim(V) = 3$ et $B = \{v_1,v_2,v_3\}$ une base de $V$. Avec les matrices $A_i \in \C^{3\times 3}$ décrites en bas et les applications  $f_i(x,y) = [x]_B^T A_i \overline{[y]_B}$, cochez ce qui s'applique.  
...
\begin{framed}\noindent 
  ...
  muni d'un produit hermitien défini positif, alors $V$ est un espace hermitien. 
\end{framed}
