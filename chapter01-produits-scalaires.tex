...
\end{displaymath}
\noindent
On dit que la forme bilinéaire est \emph{non dégénérée} si la condition suivante est vérifiée:
\begin{quote}
...
Vérifions (BL~\ref{ps:2}). Pour tout  $u,v,w ∈ K^n$ et $α ∈ K$:  
\begin{eqnarray*}
...
Soit $V$ un espace vectoriel de dimension finie et $B = \{v_1,\dots,v_n\}$ une base de $V$. Pour une forme bilinéaire $f: V × V ⟶ K$ 
 ...
 \begin{eqnarray*}
   f(x,y) & = & f \left( ∑_{i=1}^n \alpha_i v_i ,   \sum_j \beta_j v_j \right) \\
          & = & ∑_{i=1}^n \alpha_i f \left(  v_i ,   \sum_j \beta_j v_j \right) \\
...
\end{displaymath}
\begin{exercise} Soit $V$ de dimension fini et $B$ une base de $V$. 
...
\begin{proposition}
  ...
  \item $f$ est non dégénérée \label{item:13}
  \item Si $v \in V$, et si $\pscal{w,v}=0$ pour tout $w \in V$, alors $v = 0$. \label{item:14}
  \end{enumerate}
\end{proposition}

\begin{proof}
  Nous montrons \ref{item:12}) et \ref{item:13}) sont équivalentes.  Du même manière, on démontre aussi que \ref{item:12}) et \ref{item:14}) sont équivalentes.

Supposons que $\rank(A_B^f) = n$ et soit $v \in V$, $v \neq 0$. Pour $w \in V$ on a 
\begin{displaymath}
...
Si $f$ est non dégénérée, alors $x^T A_B^f \neq 0$ pour tout $x \in K^n$ tel que $x \neq 0$. Ceci implique que les lignes de $A_B^f$ sont linéairement indépendantes. Alors $\rank(A_B^f) = n$. 
\end{proof}
...
\item 	Soit $V\subseteq\R_n[x]$ l'espace euclidien des polyn\^omes de degr\'e au plus $n$. Si $f,g \in V$ on pose 
	\begin{displaymath}
		\pscal{f,g} = \int_0^{1} f(x)g(x) \, \mathrm dx.
	\end{displaymath}	
	D\'ecrire la matrice $A^f_B$ pour $B=\{1, x, \ldots, x^n\}$.
\item 	On d\'efinit $f: \R^3\times \R^3\to \R$ par	
	$$f\left(\begin{pmatrix} x_1 \\ x_2 \\ x_3 \end{pmatrix}, \begin{pmatrix} y_1\\ y_2 \\ y_3 \end{pmatrix} \right) =2x_1y_1-x_2y_3-x_3y_2+5x_1y_2+5x_2y_1-8x_3y_3.$$
	Soient $$B = \left\{ \begin{pmatrix} 1 \\ 0 \\ 0\end{pmatrix}, \begin{pmatrix} 0 \\ 1 \\ 0 \end{pmatrix}, \begin{pmatrix} 0 \\ 0 \\ 1 \end{pmatrix} \right\} 
		\text{ et } B' = \left\{ b_1' = \begin{pmatrix} 1 \\ 1 \\ 0\end{pmatrix}, b_2' = \begin{pmatrix} 2 \\ 0 \\ 0 \end{pmatrix}, b_3' = \begin{pmatrix} 0 \\ 1 \\ -1 \end{pmatrix} \right\}.$$
	\begin{itemize}
		\item[a)] Trouver la matrice $A^f_B$ par rapport \`a la base canonique $B$ de~$\R^3$.
		\item[b)] Trouver la matrice de changement de base $P_{B'B}$.
		\item[c)] Trouver la matrice $A^f_{B'}$ par rapport \`a la base $B'$ de~$\R^3$.
	\end{itemize}
\end{enumerate}
...
\begin{framed}\noindent 
  Pour ce paragraphe~\ref{sec:orthogonalite}, si il n'est pas spécifié autrement,  $V$
...
\begin{framed}\noindent 
  Pour ce paragraphe~\ref{sec:class-des-matr}, si il n'est pas spécifié autrement,  $V$
  est toujours un espace vectoriel sur $K$
  muni d'une forme bilinéaire symétrique $\pscal{,}$. 
\end{framed}
...
\end{equation}  égal à zéro si $i \neq j$. Pour la matrice de passage $P \in K^{n \times n}$ dont les colonnes sont les vecteurs $[u_1]_B,\dots,[u_n]_B$ on a alors que 
\begin{displaymath}
  P^T A_B^{\pscal{.}}P
\end{displaymath}
...
parce que  $K^n$ muni de la forme linéaire symétrique $\pscal{u,v} = u^TAv$ possède une base orthogonale. 
\end{proof}
...
\begin{equation}
  ...
  P^T A {P} =
    ...
    \end{pmatrix}. 
\end{equation}
où les  $c_i$ sont ordonnés de sorte que  $c_1,\dots, c_r >0$, $c_{r+1},\dots ,c_s<0$ et $c_{s+1},\dots , c_n = 0$. En multipliant les premières $s+r$ colonnes de $P$ par $1/ \sqrt{|c_i|}$ on obtient en fait une factorisation~\eqref{eq:29} telle que 
...
formes bilinéaire symétriques 
 définis par les matrices suivantes
  \begin{displaymath}
    \begin{pmatrix}
      1 & 2 \\
      2 & -1
...
Soit $V$ un espace vectoriel de dimension finie sur un corps $K$ muni d'une forme bilinéaire symétrique. Nous avons vu (théorème~\ref{thr:5}) que $V$ possède une base orthogonale. Supposons que cette base est $B = \{v_1,\dots,v_n\}$ et considérons $x = \sum \alpha_i v_i \in V$ et $y \in \sum_i \beta_i v_i \in V$. Le produit scalaire s'écrit 
\begin{eqnarray*}
...
\begin{theorem}[Supplémentaire orthogonal]
  \label{thr:8}
  Soient $V$ un espace vectoriel de dimension finie sur corps $K$ et $W$ un sous-espace de $V$. Soit $\pscal{.}$ un produit scalaire tel que, si restreint sur $W\times W$, il est non dégénéré. Alors $V = W \oplus W^\perp$.  
\end{theorem}
...
\item Soit $V$ un espace vectoriel de dimension finie sur un corps $K$ et soit $\pscal{.}$ un produit scalaire. Exprimez $(W_1 +W_2)^\perp$ et $(W_1\cap W_2)^\perp$ en fonction de $W_1^\perp$ et $W_2^\perp$. 
...
Pour tout $v \in V$ $\pscal{v,v} \in \R$, puisque $\pscal{v,v} = \overline{\pscal{v,v}}$, on dit que le produit hermitien  est \emph{défini positif} si $\pscal{v,v} \geq 0$ pour tout $v \in V$ et si $\pscal{v,v}>0$ lorsque $v \neq 0$. 
